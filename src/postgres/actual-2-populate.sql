CREATE OR REPLACE PROCEDURE generate_cal_actual(prior_years INT, future_years INT, pivot_date DATE DEFAULT NULL)
LANGUAGE plpgsql
AS $$
DECLARE 
    start_year INT := EXTRACT(YEAR FROM COALESCE(pivot_date, current_date)) - (1 + prior_years);
    end_year INT := EXTRACT(YEAR FROM COALESCE(pivot_date, current_date)) + 1 + future_years;
...
BEGIN
    ...
    SELECT
        ...
        DATE_TRUNC('month', datekey) AS cal_month_start_date,
